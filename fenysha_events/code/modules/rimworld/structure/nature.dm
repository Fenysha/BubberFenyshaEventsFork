...
/obj/structure/rimworld/flora
	 ...
	 * Destruction
	 *
	 * Used for plants that can be cut, burned, crushed,
	 * mined or otherwise deliberately destroyed.
	 */
	...
	/*
	 * Optional drops when a plant is uprooted instead of harvested.
	 */
...
/obj/structure/rimworld/flora/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	/*
	 * Shovel always gets priority for digging/replanting.
	 */
...
/obj/structure/rimworld/flora/proc/can_harvest(mob/user, obj/item/harvesting_item)
	...
	if(harvesting_item)
		if(harvesting_item.tool_behaviour in harvest_tools)
			return TRUE
...
/obj/structure/rimworld/flora/proc/perform_harvest(mob/living/user, product_amount_multiplier = 1)
	...
	var/list/created_products = spawn_products(
		products,
		harvest_amount_low,
		harvest_amount_high,
	...
	/*
	 * Seeds are deliberately separate from the main harvest.
	 * This allows food/resource plants to have a chance to reproduce.
	 */
	if(LAZYLEN(seed_products) && prob(seed_chance))
		var/list/seeds = get_weighted_product_list(seed_products, 1, 1)
		created_products += spawn_products(
			seeds,
			1,
			1,
			product_amount_multiplier
...
/obj/structure/rimworld/flora/proc/can_destroy(mob/user, obj/item/tool)
	...
	if(tool.tool_behaviour in destroy_tools)
		return TRUE
...
/obj/structure/rimworld/flora/tree
	...
	var/stump_type = /obj/structure/rimworld/flora/tree/stump

	var/list/fall_products


/obj/structure/rimworld/flora/tree/Initialize(mapload)
	. = ..()
...
/obj/structure/rimworld/flora/tree/can_destroy(mob/user, obj/item/tool)
	if(falling)
		return FALSE
...
/obj/structure/rimworld/flora/tree/destroy(mob/living/user, obj/item/tool)
	if(falling)
		return FALSE
...
/obj/structure/rimworld/flora/tree/proc/fell(mob/living/user, obj/item/tool)
	if(falling)
		return FALSE
	...
	/*
	 * Pick the side on which the tree will fall.
	 *
	 * A matrix is used instead of changing dir, so large sprites
	 * can smoothly rotate during the fall.
	 */
	...
	animate(
		...
		pixel_x = fall_pixel_x,
		pixel_y = fall_pixel_y,
		time = fall_time
	)
...
/obj/structure/rimworld/flora/tree/proc/finish_fall()
	if(QDELETED(src))
		return
	...
	/*
	 * The stump remains on the original tile.
	 */
	if(stump_type)
		var/obj/structure/rimworld/flora/tree/stump/new_stump = new stump_type(tree_turf)
		new_stump.name = "[name] stump"

	/*
	 * Trees convert into physical material only after
...
/obj/structure/rimworld/flora/tree/stump/can_harvest(mob/user, obj/item/harvesting_item)
	return FALSE


/obj/structure/rimworld/flora/tree/stump/uproot(mob/living/user)
	anchored = FALSE
	uprooted = TRUE

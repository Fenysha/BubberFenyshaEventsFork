/obj/structure/chair/buckshot
    name = "buckshot chair"
    desc = "A sturdy chair with an integrated system of blood transfer and defibrillators."
    icon_state = "echair1"
...
/obj/structure/chair/buckshot/buckle_mob(mob/living/M, force, check_loc)
    ...
    if(party.game_started)
        to_chat(M, span_warning("You cannot sit on the chair after the game has started!"))
        return
    return ..()
...
/obj/structure/chair/buckshot/attack_tk(mob/user)
    return

/obj/structure/chair/buckshot/proc/register_player(datum/buckshot_roulette_party/party_instance)
    ...
    player.AddComponent(/datum/component/buckshot_roulette_participant, party_instance, ctr)
    return player
...
/obj/structure/chair/buckshot/wrench_act_secondary(mob/living/user, obj/item/weapon)
    to_chat(user, span_warning("You attempt to dismantle the resuscitation system!"))
    user.balloon_alert(user, "Dismantling [src]!")
    if(!do_after(user, 1 MINUTES, src))
        return
    to_chat(user, span_userdanger("A powerful electric shock strikes your heart!"))
    electrocute_mob(user)
    var/obj/item/organ/heart/heart = user.get_organ_slot(ORGAN_SLOT_HEART)
    if(heart)
        heart.Stop()
    user.adjust_fire_loss(150, forced = TRUE)
    user.throw_at(get_ranged_target_turf(src, REVERSE_DIR(src.dir), rand(2, 3)), 2, 3, src, FALSE)
    return TRUE
...
/obj/structure/crt_mechanims
    ...
    icon = 'fenysha_events/icons/structures/buckshot/crt.dmi'
    icon_state = "crt0"
    base_icon_state = "crt"
    obj_flags = INDESTRUCTIBLE | BOMB_PROOF | LAVA_PROOF | FIRE_PROOF

    // Reference to the participant component
    var/datum/component/buckshot_roulette_participant/participant_comp = null
...
/obj/structure/crt_mechanims/proc/set_participant(datum/component/buckshot_roulette_participant/comp)
...
/obj/structure/crt_mechanims/proc/revive_player()
    ...
    if(player.stat != DEAD)
        return
    playsound(src, 'fenysha_events/sounds/effects/buckshot/defib_discharge.ogg', 50, 1)
    sleep(1.5 SECONDS)
    player.notify_revival("Your heart is being defibrillated!")
    player.grab_ghost() // Return ghost to body

    player.revive(HEAL_DAMAGE | HEAL_ORGANS | HEAL_BLOOD | HEAL_WOUNDS | HEAL_TRAUMAS)
    ...
    player.flash_act()
    log_game("[key_name(player)] was forcibly revived by buckshot crt device.")
    to_chat(player, span_userdanger("Your heart explodes back to life! You're back in the game!"))
    SEND_SOUND(player, 'fenysha_events/sounds/effects/buckshot/heartbeat_effect.ogg')


/obj/structure/table/buckshot_table_part
    // icon_state is set dynamically
    icon = 'fenysha_events/icons/structures/buckshot/buckshot_table.dmi'
    icon_state = ""
    base_icon_state = ""
...
/obj/structure/table/buckshot
    ...
    obj_flags = INDESTRUCTIBLE | BOMB_PROOF | LAVA_PROOF | FIRE_PROOF
    // Party attached to the table
    var/datum/buckshot_roulette_party/party = null
    // Table parts for the 3x3 table structure
    var/list/parts
...
/obj/structure/table/buckshot/Destroy(force)
    . = ..()
    if(party)
        INVOKE_ASYNC(party, TYPE_PROC_REF(/datum/buckshot_roulette_party, end_game))
    for(var/obj/structure/table/buckshot_table_part/part in parts)
        if(!QDELETED(part))
            qdel(part)
    for(var/dir in crts_by_dirs)
        var/atom/crt = crts_by_dirs[dir]
        if(crt && !QDELETED(crt))
            qdel(crt)
    parts = null
...
/obj/structure/table/buckshot/proc/build_table()
    ...
    for(var/i = -1; i <= 1; i++)
        for(var/j = -1; j <= 1; j++)
            ...
            part.name = name
            part.desc = desc
            part.forceMove(locate(src.x + i, src.y + j, src.z))
...
/obj/structure/box_with_item/Initialize(mapload, count, mob/living/carbon/human/owner, datum/buckshot_roulette_party/party_instance)
    . = ..()
    to_give = count
    player = owner
    party = party_instance
    if(party_instance)
        party_instance.created_item_boxes += src

/obj/structure/box_with_item/Destroy(force)
    . = ..()
    if(party)
        party.created_item_boxes -= src

/obj/structure/box_with_item/attack_hand(mob/living/user, list/modifiers)
    . = ..()
    if(!party || !party.is_participant(user))
        to_chat(user, span_warning("You are not a participant in the game!"))
        return
    if(user != player)
    ...
    if(given_count >= to_give)
        ...
        qdel(src)
        return
    if(!party.can_give_item_to_player(user))
        to_chat(user, span_warning("You cannot take any more items from the box!"))
        qdel(src)
        return
    ...
    playsound(src, 'fenysha_events/sounds/effects/buckshot/item_pickup.ogg', 50, TRUE)
    if(given_count >= to_give)
        to_chat(user, span_notice("You took the last item from the box."))
        qdel(src)

/obj/structure/table/buckshot/proc/create_item_boxes(items_per_player)
    for(var/mob/living/carbon/human/player in party.get_players())
        ...
        if(!target_turf || !party.can_give_item_to_player(player))
            continue
        new /obj/structure/box_with_item(target_turf, items_per_player, player, party)
...
/obj/structure/table/buckshot/proc/try_start_game(mob/user)
    ...
    if(party.game_started)
        ...
        balloon_alert(user, "Game already started!")
        return
    if(!istype(user))
        return
    INVOKE_ASYNC(party, TYPE_PROC_REF(/datum/buckshot_roulette_party, attempt_start_game), user)

...
/obj/structure/chair/buckshot/wrench_act_secondary(mob/living/user, obj/item/weapon)
	to_chat(user, span_warning("Ты пытаешься демонтировать систему реанимации!"))
	user.balloon_alert("Демонтирует [src]!")
	if(do_after(user, 1 MINUTES, src))
		return
	to_chat(user, span_userdanger("Мощный разряд тока поражает твое сердце!"))
...
/obj/structure/box_with_item/attack_hand(mob/living/user, list/modifiers)
	...
	if(!party.can_give_item_to_player(user))
		to_chat(user, span_warning("Ты не можешь взять больше предметов из ящика!"))
		qdel(src)
		return
	...
	playsound(src, 'fenysha_events/sounds/effects/buckshot/item_pickup.ogg', 50, 1)
	if(given_count >= to_give)

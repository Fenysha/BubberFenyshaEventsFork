...
/datum/component/bossbar/proc/show_to(client/C)
	...
	M.hud_used.screen_groups[HUD_GROUP_STATIC] += base
	M.hud_used.screen_groups[HUD_GROUP_STATIC] += fill
	M.hud_used.screen_groups[HUD_GROUP_STATIC] += portrait
...
/datum/component/bossbar/proc/hide_from(client/C)
	...
	if(H)
		H.screen_groups[HUD_GROUP_STATIC] -= screens[1]
		H.screen_groups[HUD_GROUP_STATIC] -= screens[2]
		H.screen_groups[HUD_GROUP_STATIC] -= screens[3]
...
/datum/component/bossbar/proc/update_all()
	...
	for(var/client/C in client_to_screens.Copy())
		...
		var/mob/M = C?.mob
		if(!M?.hud_used || M.hud_used != screens[4] || !(screens[1] in M.hud_used.screen_groups[HUD_GROUP_STATIC]))
			hide_from(C)
			show_to(C)
			continue
